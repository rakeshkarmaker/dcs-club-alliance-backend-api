...
@Injectable()
export class AuthRepository extends BaseRepository<UserAuth, CreateAuthDto, any> implements IAuthRepository {
    constructor(private readonly prismaService: PrismaService) {
        super(prismaService.userAuth);
    }

    async findUserByEmail(email: string): Promise<UserAuth |null> {
        const user = await this.prismaService.userAuth.findUnique({ where: { email } });
        if (!user) return null;
        return user;
    }
