  ...
  implements NestInterceptor<T, Response<T>>
{
  intercept(
    ...
    return next.handle().pipe(
      // Here, we use the RxJS map operator to transform the response. The next.handle() method returns an Observable, which we can manipulate. The pipe method allows us to chain multiple operators to the Observable.
      map((data) => ({
        // The map operator takes the data emitted by the Observable and transforms it into a new object that matches the Response<T> interface.
        success: true,
        data,
